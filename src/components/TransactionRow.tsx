...
export function TransactionRow({ tx, onPress, showDate = true }: { tx: Transaction; onPress?: () => void; showDate?: boolean }) {
  ...
  const confirmDelete = () => {
    ref.current?.close();
    Alert.alert('Remove this entry?', `${formatMoney(tx.amount, currency)} · ${tx.note ?? category?.name ?? ''}`, [
      { text: 'Keep', style: 'cancel' },
      { text: 'Remove', style: 'destructive', onPress: () => remove(tx.id) },
    ]);
  };
  ...
  const renderRight = () => (
    <View style={styles.actions}>
      {onPress ? (
        <Pressable accessibilityRole="button" accessibilityLabel="Edit" onPress={() => { ref.current?.close(); onPress(); }} style={[styles.action, { backgroundColor: colors.accentSoft }]}>
          <Ionicons name="pencil" size={20} color={colors.accent} />
        </Pressable>
      ) : null}
      <Pressable accessibilityRole="button" accessibilityLabel="Delete" onPress={confirmDelete} style={[styles.action, { backgroundColor: colors.dangerSoft }]}>
        <Ionicons name="trash-outline" size={20} color={colors.danger} />
      </Pressable>
    </View>
  );

  const title = tx.note || category?.name || (tx.type === 'income' ? 'Income' : 'Expense');
  const subtitleParts = [category?.name, showDate ? friendlyDate(tx.occurredAt) : null, tx.isRecurringInstance ? 'repeats' : null].filter(Boolean);
